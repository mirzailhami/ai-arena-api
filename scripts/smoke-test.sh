...
#
# JWT source: platform-ui DevTools → Application → Cookies → 'v3jwt'
# =============================================================================
...
# ── 10. List tourneys (empty) ─────────────────────────────────────────────────
info "10. GET /tourneys (expect 200)"
...
info "11. POST /tourneys — create tournament with bracket"
...
if [[ -n "${TOURNEY_ID:-}" ]]; then
  info "12. GET /tourneys/${TOURNEY_ID}"
...
# ── 14. Delete tourney ────────────────────────────────────────────────────────
if [[ -n "${TOURNEY_ID:-}" ]]; then
  info "14. DELETE /tourneys/${TOURNEY_ID} (expect 204)"
